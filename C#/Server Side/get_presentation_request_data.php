...
{    
    ...
    $statement = $user->getPresentationRequestData($maxNumRowsToGet, $startRowOffset);
    ...
    {        
        ...
        {            
            ...
            // NOTE: the name of the key (for example the key "Email") MUST match the name of the property of the PresentationRequestViewModel class back in the Xamarin application.
            // This is because, when the JsonConvert.DeserializeObject() function is called in the application, it will look for keys in this array that correspond to properties of the PresentationRequestViewModel object.
            ...
            $request = array(
                ...
                "OrgZip" => $row['orgZip'],
                "AnyPres" => $row['anyPres'],
                "InvisiblePres" => $row['invisiblePres'],
                "MentorPres" => $row['mentorPres'],
                "StarPres" => $row['starPres'],
                "WaterPres" => $row['waterPres'],
                "PlanetPres" => $row['planetPres'],
				"MarsPres" => $row['marsPres'],
                "StationPres" => $row['stationPres'],
				"TelescopePres" => $row['telescopePres'],
                "NANOGravPres" => $row['NANOGravPres'],
                "presRotations" => $row['presRotations'],
                "NoHandsOn" => $row['noHandsOn'],
                "BuildMolecule" => $row['buildMolecule'],
                "DesignAlien" => $row['designAlien'],
                "PocketSolSys" => $row['pocketSolSys'],
                "SizeUpMoon" => $row['sizeUpMoon'],
                "TelescopeDesign" => $row['telescopeDesign'],
                "RoboticArm" => $row['roboticArm'],
                "ElectroWar" => $row['electroWar'],
                "BuildWatershed" => $row['buildWatershed'],
                "DropInBucket" => $row['dropInBucket'],
                "GradeLevels" => $row['gradeLevels'],
                "NumberStudents" => $row['numberStudents'],
                "ProposedDateTime" => $row['proposedDateTime'],
                "ProjectSupplies" => $row['projectSupplies'],
                "SpeakerSupplies" => $row['speakerSupplies'],
                "CompSupplies" => $row['compSupplies'],
                "CordSupplies" => $row['cordSupplies'],
                "MicSupplies" => $row['micSupplies'],
                "OtherSupplies" => $row['otherSupplies'],
                "TravelFee" => $row['travelFee'],
                "AgreeStatement" => $row['agreeStatement'],
                "Concerns" => $row['concerns'],
                "CollegePres" => $row['collegePres'],
                "SkypePres" => $row['skypePres'],
                "AltOtherPres" => $row['altOtherPres']);
        ...
        //
        //      List<PresentationRequestViewModel> rvmList = JsonConvert.DeserializeObject<List<PresentationRequestViewModel>>(responseContent); // Deserialize (as in unencode) the JSON array and construct a List of PresentationRequestViewModel objects from it.
