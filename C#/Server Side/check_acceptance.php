...
{
    $email = $presentationID = $isAccepted = "";

    if(isset($_POST["email"]) && isset($_POST["presentationID"]))
	{
        ...
        $statement = $user->checkAcceptance($presentationID);
        ...
        if (in_array(null, $email) || in_array("", $email))
            $isAccepted = false;
        else
            $isAccepted = true;
